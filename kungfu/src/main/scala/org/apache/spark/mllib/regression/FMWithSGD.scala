...
object FMWithSGD {
  ...
  def train(input: RDD[LabeledPoint],
            ...
            initStd: Double,convergenceTol:Double = 0.001): (FMModel,Array[Double]) = {
    new FMWithSGD(task, stepSize, numIterations, dim, regParam, miniBatchFraction)
      .setInitStd(initStd)
      .run(input,convergenceTol)
  }

  def train(input: RDD[LabeledPoint],
            task: Int,
            numIterations: Int): (FMModel,Array[Double]) = {
...
class FMWithSGD(private var task: Int,
  ...
  //changed by zqwu
  //private var minLabel: Double = Double.MinValue
  //private var maxLabel: Double = Double.MaxValue
  ...
  /**
    ...
    */
  def run(input: RDD[LabeledPoint],convergenceTol:Double = 0.001): (FMModel,Array[Double]) = {
    println(input.first())
    ...
    //changed by zqwu
    val (weights,loss) = GradientDescent.runMiniBatchSGD(data,gradient,
      updater,stepSize,numIterations,0.0,miniBatchFraction,initWeights,convergenceTol)
    //val weights = optimizer.optimize(data,initWeights)
    println("================optimizer============")
    println("optimizer is " + optimizer.toString)
    println("================optimizer============")
